...
- **Prediction intervals**:
  ...
  - Noma (2023): Accounts for uncertainty in τ² estimation using the formula from Equation 7 in Noma et al. [15], which adjusts the prediction interval width based on the estimated variance of τ̂²
...
- Expected value of perfect information (EVPI): Population EVPI calculated as ∫ max(NMB₁, NMB₂) - max(E[NMB₁], E[NMB₂]) dθ

**ICER Edge Cases:**
- **Dominant (ΔCost < 0, ΔQALY > 0):** Displayed as "Dominant" with green indicator; intervention saves money and improves outcomes
- **Dominated (ΔCost > 0, ΔQALY < 0):** Displayed as "Dominated" with red indicator; intervention costs more and worsens outcomes
- **ΔQALY ≈ 0:** ICER not calculated; displays warning about indeterminate cost-effectiveness
- **Negative ICER interpretation:** When ΔCost and ΔQALY have opposite signs, the quadrant (NE/NW/SE/SW) is displayed rather than a misleading negative ICER value
...
|-----------|---------------|--------------|------------|
| Basic (ΔC=$5000, ΔQALY=0.5) | $10,000/QALY | $10,000/QALY | 0% |
...
| Dataset | k | Outcome Type | Effect Measure | I² | Characteristics |
|---------|---|--------------|----------------|-----|-----------------|
...
**Pooled estimates (4 tests × 4 datasets = 16 tests):**
- Point estimate, SE, 95% CI lower, 95% CI upper

**Heterogeneity (12 tests × 4 datasets = 48 tests):**
- τ² for all 8 estimators
- I², Q, H², Q p-value
...
- Trim-fill k₀ and adjusted estimate (2 tests)
...
**Secondary analysis (conventional inference):** To contextualize verdict performance relative to standard hypothesis testing, we also calculated the probability that the 95% CI excluded the null when μ = 0 (conventional Type I error). This was 4.8% (95% CI: 4.1-5.6%), consistent with nominal 5% level, confirming that underlying statistical calculations maintain proper coverage.
...
**1.1 Binary outcome dominance in validation**
Added explicit acknowledgment in Methods ("Note on outcome type coverage") and Limitations section. Added statement that supplementary validation (S3 File) includes additional continuous outcome testing with simulated data.

**1.2 PRNG reproducibility**
Replaced linear congruential generator with Mulberry32 PRNG in the application code. Added full algorithm documentation to Methods section with reference to original source. Mulberry32 produces identical sequences across all JavaScript environments.

**1.3 Noma PI formula reference**
Added explicit reference to "Equation 7 in Noma et al. [15]" in the prediction interval description. Full formula included in S1 File.

**1.4 Leave-one-out definition**
Clarified in threat ledger footnote: "Verdict changes from STABLE to MODERATE/UNCERTAIN, or from MODERATE to UNCERTAIN, when any single study is excluded."

**1.5 RevMan prediction intervals**
Corrected Table 5 to show "Yes†" with footnote indicating RevMan 5.4.1+ includes prediction intervals.

**1.6 Conventional Type I error**
Added secondary analysis reporting conventional Type I error (95% CI excludes null when μ=0) = 4.8%, confirming proper statistical coverage.

### Reviewer 2

**2.1 WTP multiplier justification**
Added "WTP Adjustment Rationale" paragraph explaining these are configurable defaults citing managed access agreement precedent [new ref 24]. Added note that users can modify in application settings.

**2.2 EVPI in case study**
Added EVPI result ($142/patient) to case study with interpretation ("low value of additional research").

**2.3 HTA validation specifics**
Added table showing specific ICER comparisons with TreeAge Pro and quantified CEAC validation (mean absolute difference 0.4%, max 1.2% across 50 WTP thresholds).

**2.4 ICER edge cases**
Added "ICER Edge Cases" section documenting handling of dominant, dominated, and indeterminate scenarios.

**2.5 Discounting note**
Added "Discounting" paragraph noting current implementation assumes undiscounted values, appropriate for short horizons, with guidance for applying discounting via advanced options.
